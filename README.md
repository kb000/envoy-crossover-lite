...
Add `crossover` as a sidecar container to your Envoy. Update configmaps via kubectl or helm to load changes into Envoy in near-realtime. That's all you need to get started, really!
...
It fetches well-known K8s `ConfigMap` resources via Kubernetes' REST API, writes config files for Envoy, and renames the files so that Envoy can atomically reconfigure itself.
...
```
[ Envoy ]--reads-->[ xDS YAML file ]<--writes--[ Crossover ]--reads-->[ ConfigMap ]<--write--[ Kubectl, Helm, etc. ]
```
...
It continuously reads Kubernetes ConfigMap resources to generate and feed xDS resources to Envoy via the local filesystem.
...
Thanks to Envoy's inotify support and Kubernetes' Watch API, any changes made via ConfigMap are still recognizable by Envoy in near real-time.
...
Usage of ./crossover:
  ...
  -output-dir string
    	Directory to output xDS configs so that Envoy can read
